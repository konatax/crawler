...
def deal(info, j_key):
    ...
    # print(j_key)
    # print(resp['info'])
    # 把resp['info']当成dom来使用   <td><img src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAB
    tree = etree.HTML(info)
    tds = tree.xpath('//td')
    result = 0
    for td in tds:
        cl = td.xpath('.//img/@class')
        num = []     #数字
        change = []   #偏移数
        for i in range(len(cl)):  #判断数字是否被隐藏
            if j_key not in cl[i]:
                n = td.xpath('./img[%d+1]/@src' % i)[0]   #数字
                d = td.xpath('./img[%d+1]/@style' % i)[0] #偏移数
                num.append(str(img2num[n]))  #方便后面拼接起来
                change.append(margin2num[d])
            else:
                continue
        source = num.copy()   #新的列表用于存放未修正位置的数字
        # i是下标  c是偏移数
        for i, c in enumerate(change):
            if c != 0:  #如果数字有偏移，就赋值给另一列表偏移前的下标元素 即 把数字放在正确的位置
                num[i+c] = source[i]
        num = int(''.join(num))
        result += num
...
def main():
    result = 0
    for i in range(1, 6):
        url = 'https://match.yuanrenxue.com/api/match/4?page=%d' % i
        headers = {
        ...
        funcName = 'getKey'
        print(key + value)
        # btoa(data.key + data.value).replace(/=/g, '') bota等同于base64.b64encode
        # base64encode是将字节转换成base64字节，因此需要将字符串编码成字节再b64encode
        s = base64.b64encode((key + value).encode())
        # print(s)
        # 将字节解码成字符再replace
        s = s.decode().replace('=', '')
        j_key = ctx.call(funcName, s)
        num = deal(info, j_key)
        result += num
    print(result)
