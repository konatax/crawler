...
$.ajax({
                    ...
                    datas = data.info;                        $('.number').text('').append(datas);
                        var j_key = '.' + hex_md5(btoa(data.key + data.value).replace(/=/g, ''));  // 这里是 = 换成 ''   /g是全局匹配的意思
                        $(j_key).css('display', 'none');
//移除img_number类名再添加img_numsber类名，相当于没变
...
意思是将class='number'的文本内容清空并填入datas

![1666158938013](C:\Users\konata\AppData\Roaming\Typora\typora-user-images\1666158938013.png)

![1666158963277](C:\Users\konata\AppData\Roaming\Typora\typora-user-images\1666158963277.png)

datas就是返回值的info的内容

![1666159013915](C:\Users\konata\AppData\Roaming\Typora\typora-user-images\1666159013915.png)

```javascript
//定义j_key
var j_key = '.' + hex_md5(btoa(data.key + data.value).replace(/=/g, ''));
//对j_key设置样式 css属性值与j_key相同就不显示（display设为none
$(j_key).css('display', 'none');
```
...
btoa 相当于base64.b64encode，将杂乱无章的字节转换为base64格式的字节，这里data.key, data.value都是字符，先encode('utf-8')，经过base64编码再传入加密函数，再decode('utf-8')，再replace('=', '')
...
class='numbers' 的内容就是返回的data.info，包含数字图片内容

'<td><img .....>...............</td>'  

有多对td，每对td中是一组数字图片，需要用j_key与img标签中的class属性值比对，如果里面有j_key的值，就不显示该图片

还需要获取图片的偏移值 @style

不需要正则，直接用xpath提取

因为data.info的内容符合网页的格式



需要将0~9 每个数字对应src放入大字典中，方便取值
...
首先，获取到某一组数字比如 num = [‘1’，‘0’，‘3’，‘4’] （原本是int，为方便拼接，直接转换为str)

该组数字偏移量 change = [0,0,1,-1]

先复制一组同样的数字 source = num.copy()

for i, c in enumerate(change):  #i是下标，c是偏移量

​      if c!= 0:   #如果有偏移   

#如果是正数即向左偏移，那么数字下标应该增大

#如果是负数即向右偏移，那么数字下标应该减小

#都是i+c

​	      num[i+c] = source[i]

num = int(''.join(num))   #此时把这组数字拼成一个数字	   
